...
function patchSource(source) {
  ...
  out = out.replace(
    'const iframeHtml=await readText(ir);const pm=iframeHtml.match(/data-id=["\']([^"\']*player\\.php\\?(?:u|vid)=[^"\']+)["\']/i)||iframeHtml.match(/["\']([^"\']*player\\.php\\?(?:u|vid)=[^"\']+)["\']/i);if(!pm)return{urls:[],diag:`iframeHTTP=${ir.status} • player=no • iframeLen=${iframeHtml.length}`};const playerUrl=joinUrl(`${NH_BASE}/`,pm[1]);if(!playerUrl)return{urls:[],diag:"playerURL=invalid"};const playerHeaders={...browserHeaders(iframeUrl),',
    'const iframeHtml=await readText(ir);const pm=iframeHtml.match(/data-id=["\']([^"\']+)["\']/i)||iframeHtml.match(/["\']([^"\']*player\\.php[^"\']*)["\']/i);if(!pm)return{urls:[],diag:`iframeHTTP=${ir.status} • player=no • iframeLen=${iframeHtml.length}`};const playerUrl=joinUrl(`${NH_BASE}/`,pm[1]);if(!playerUrl)return{urls:[],diag:"playerURL=invalid"};let sessionCookie="";try{const sc=ir.headers&&ir.headers.get?ir.headers.get("set-cookie"):"";if(sc)sessionCookie=sc.split(";")[0];}catch(_){}const playerHeaders={...browserHeaders(iframeUrl),...(sessionCookie?{Cookie:sessionCookie}:{}),'
  );

  if (!out.includes("function baseMediaId") || !out.includes("episodeFromInputId") || !out.includes("sessionCookie") || !out.includes("data-id")) return null;
  return out;
}
