...
`function normalizeMediaUrl(value,base){let u=decodeHtml(String(value||"")).replace(/\\\\\//g,"/").replace(/\\\//g,"/").trim();if(!u)return"";try{u=decodeURIComponent(u);}catch(_){}if(/^\\/\\//.test(u))u="https:"+u;else if(/^\\//.test(u))u="https://nhplayer.com"+u;else if(!/^https?:\\/\\//i.test(u)&&base){try{u=new URL(u,base).href;}catch(_){}}return /^https?:\\/\\//i.test(u)?u:"";}\nasync function nhplayerStreams(html){
    ...
    if(oldU){
      ...
      if(playerUrl){
        ...
        if(p&&p.ok){
          ...
          let rawFile="";
          const text=String(ph||"").replace(/\\\\\//g,"/").replace(/\\\//g,"/");
          ...
          if(!rawFile&&/^\\s*(?:https?:)?\\/\\//i.test(text))rawFile=text.trim().replace(/^['"]|['"]$/g,"");
