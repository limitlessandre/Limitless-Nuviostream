...
function getSeasonRegexes(season) {
  if (season === 1) {
    return { mustNot: [/season\s*[2-9]/i, /[\s\-][iI]{2,}/, /\s+[2-9]nd/i, /\s+[2-9]rd/i, /\s+[2-9]th/i, /\s+ii/i, /\s+iii/i, /\s+iv/i, /\s+v/i] };
  }
  const patterns = [];
...
function matchCard(cards, jikanTitle, baseTitle, season) {
  ...
  for (const card of cards) {
    ...
    for (const title of card.titles) {
      if (normalize(title).includes(normalizedBase)) { matchesBase = true; break; }
    }
    ...
    if (season === 1) {
      ...
      for (const title of card.titles) {
        if (seasonRules.mustNot.some((regex) => regex.test(title))) { hasOtherSeason = true; break; }
      }
      if (!hasOtherSeason) seasonMatches = true;
    } else {
      for (const title of card.titles) {
        if (seasonRules.must.some((regex) => regex.test(title))) { seasonMatches = true; break; }
      }
    }
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      if (mediaType === "tv" && mapping) searchQuery = mapping.anime_title || animeTitle.split(":")[0].trim();
      else searchQuery = animeTitle.split(":")[0].trim();
      ...
      let animeSlug = null;
      if (cards.length > 0) {
        if (mediaType === "tv") animeSlug = matchCard(cards, animeTitle, mapping && mapping.anime_title || animeTitle, season);
        else animeSlug = matchMovieCard(cards, animeTitle);
      }
      if (!animeSlug) {
      ...
      if (!masterUrl) {
        ...
        if (matches) masterUrl = matches[0];
      }
      const subtitles = [];
      $epPage("track").each((i, el) => {
        ...
        if (src && (kind === "subtitles" || kind === "captions" || src.endsWith(".ass") || src.endsWith(".vtt"))) {
          const label = $epPage(el).attr("label") || "English";
          subtitles.push({
            url: src,
            name: `${label} [SOFTSUB]`,
      ...
      if (masterUrl) {
        const hasDub = format.includes("Dub");
        const tag = hasDub
        ...
        streams.push({
          name: `AniZone${tagText}`,
          title: `${animeTitle} - Episode ${mappedEp}${tagText}`,
          url: masterUrl,
          quality: "Multi",
          headers: HEADERS,
          subtitles
