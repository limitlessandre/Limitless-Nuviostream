...
function subtitleTracks(metadata, headers) {
  const raw = Array.isArray(metadata && metadata.subtitles)
    ? metadata.subtitles
    : (Array.isArray(metadata && metadata.subtitleTracks) ? metadata.subtitleTracks : []);
  const seen = new Set();
  const rows = [];
  for (let i = 0; i < raw.length; i++) {
    ...
    const label = clean(track.label || track.title || track.name) || (language === "en" ? "English" : language);
    rows.push({
      id: `hstream-${language}-${i}`,
      url,
      language,
      lang: language,
      name: `${PROVIDER} • ${label}`,
...
async function getStreams(inputId, mediaType, season, episode) {
  try {
    ...
    const subtitles = subtitleTracks(providerMeta, headers);
