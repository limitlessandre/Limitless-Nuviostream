...
function diag(stage, detail) {
  ...
  return [{ name: text, title: text, url: "https://hentaihaven.xxx/favicon.ico", quality: "DIAG", language: "Unavailable", provider: PROVIDER, type: "m3u8", subtitles: [] }];
}
...
function typeFromUrl(url, hinted) {
  ...
  if (hint === "mp4" || hint === "m3u8") return hint;
  const value = clean(url).toLowerCase();
  if (value.includes(".mp4")) return "mp4";
  return "m3u8";
}
function playbackHeaders(payload) {
  const supplied = payload && payload.headers && typeof payload.headers === "object" ? payload.headers : null;
  if (supplied) return supplied;
  if (clean(payload && payload.backend).toLowerCase() === "xxx") {
    return { Referer: "https://hentaihaven.xxx/wp-admin/admin-ajax.php", Origin: "https://hentaihaven.xxx" };
  }
