...
const snapshot = { titles: [
  ...
  { id: 'sp:test:breasts', type: 'series', adult: true, title: 'Big Boobs Test', titles: { english: null, romaji: 'Big Boobs Test', japanese: null, aliases: [] }, genres: [], tags: ['big boobs'], censorStatus: 'unknown', releaseDate: '2024-02-01' },
...
test('manifest exposes four parent catalogs and child genre options', () => {
...
test('latest and all ignore encoded search extras', () => {
  assert.deepEqual(catalogMetas(snapshot, parseCatalogRequest(new URL('https://test/catalog/series/scarlet-peach-latest/search=naruto.json'))).map((meta) => meta.id), ['sp:test:breasts', 'sp:test:milf', 'sp:test:rape', 'sp:test:minor-coded', 'mal:49095', 'mal:368']);
