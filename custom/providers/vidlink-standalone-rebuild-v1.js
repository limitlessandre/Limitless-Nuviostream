...
// Nexus-owned implementation of the measured Haylox/Vidlink control contract.
// See docs/vidlink-standalone-rebuild-baseline.md for provenance and comparison.
// Keep extraction headers off returned rows: that is how the confirmed control
// hands direct media to Nuvio. API proxy flags/captions are not baseline features.
const PROVIDER_NAME = "Vidlink Standalone Rebuild";
...
function candidate(url, qualityValue) {
  ...
  return { name: "Vidlink", title: "Vidlink", url, quality,
...
function extract(data) {
  if (!data) return [];
  const rows = [];
  const add = (url, quality) => { const row = candidate(url, quality); if (row) rows.push(row); };
  ...
  if (stream && stream.qualities) {
    ...
    for (const item of data.streams) if (item && item.url) add(item.url, item.quality || item.resolution || null);
  } else if (Array.isArray(data.links)) {
    for (const item of data.links) if (item && item.url) add(item.url, item.quality || null);
  } else {
    // Preserve the control's legacy response support, without adding candidates.
    const visit = object => {
      ...
      for (const key of Object.keys(object)) {
        ...
        if (typeof value === "string" && value.startsWith("http")) {
          if (![".srt", ".vtt", "subtitle", "caption"].some(part => value.includes(part))) add(value, key);
    ...
    for (const line of text.split("\n").map(value => value.trim()).filter(Boolean)) {
      if (line.startsWith("#EXT-X-STREAM-INF:")) {
        ...
        if (!line.startsWith("http")) {
          try { resolved = new URL(line, url).toString(); } catch (_) {}
        }
...
function present(rows) {
  const seen = new Set();
  const accepted = rows.filter(row => QUALITY_HEIGHTS[row.quality] && row.url && row.url.startsWith("https"))
  ...
  return accepted.map((row, index) => {
    // The control's title sort prefix is retained byte-for-byte; only the visible
    // name changes to identify this unconfirmed rebuild beside the control.
    const sortTag = (index + 1).toString(2).padStart(20, "0").replace(/0/g, "\u200B").replace(/1/g, "\uFEFF");
    const height = QUALITY_HEIGHTS[row.quality];
    const tier = height >= 2160 ? "4K" : height >= 1440 ? "Enhanced QHD" : height >= 1080 ? "FHD" : "HD";
    return { ...row, name: `${PROVIDER_NAME} • ${tier} ${height}p • [UNK]`, title: sortTag + "Vidlink" };
  });
}
...
async function getStreams(tmdbId, mediaType, season, episode) {
  try {
    ...
    if (!metadata || !(isTv ? metadata.name : metadata.title) || !encryption || !encryption.result) return [];
    const endpoint = isTv ? `${API_BASE}/tv/${encryption.result}/${season}/${episode}` : `${API_BASE}/movie/${encryption.result}`;
    const response = await fetch(endpoint, { headers: HEADERS });
    if (!response.ok) return [];
    const extracted = extract(await response.json());
    const direct = extracted.filter(row => !row._playlist);
    const playlists = await Promise.all(extracted.filter(row => row._playlist).map(row => playlistRows(row.url)));
