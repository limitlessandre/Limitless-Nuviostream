...
// Re:ANIME Nexus presentation wrapper using NAMING_STANDARDS.md.
// Extraction/playback stays in the validated season-aware provider; this file
// only normalizes quality and evidence-based audio/subtitle tags.
const PROVIDER_NAME = "Re:ANIME";
const BASE_URL = "https://raw.githubusercontent.com/limitlessandre/Limitless-Nuviostream/refs/heads/Limitless-nexus/custom/providers/reanime-season-aware-v1.js?rev=180";
let cached = null;
...
function hasMultipleAudio(row) {
  const direct = [row && row.audioTracks, row && row.audios];
  if (direct.some(group => Array.isArray(group) && group.filter(Boolean).length > 1)) return true;
...
function normalizeRow(row) {
  ...
  return { ...row, name: `${PROVIDER_NAME} • ${qualityLabel(height)} • ${classification(row)}` };
}
