...
// 1Shows Nexus probe v0.1.6
// Adds transport validation and HLS master expansion for decrypted Vidrock URLs.
...
const TRANSPORT_TIMEOUT_MS = 6500;
...
function vidrockPlaybackHeaders() { return {"User-Agent":USER_AGENT,"Referer":`${VIDROCK_BASE}/`}; }
...
async function fetchTransport(url, headers) {
  ...
  try {
    if(typeof AbortController!=="undefined") {
      ...
      if(typeof setTimeout==="function") timer=setTimeout(()=>{ try{controller.abort();}catch(_){} },TRANSPORT_TIMEOUT_MS);
    }
    const r=await fetch(url,{headers:{"User-Agent":USER_AGENT,"Accept":"application/vnd.apple.mpegurl,application/x-mpegURL,video/*,*/*",...(headers||{})},redirect:"follow",skipSizeCheck:true,signal:controller?controller.signal:undefined});
    if(timer) clearTimeout(timer);
    if(!r) return {ok:false,status:0,error:"no response",url:clean(url),contentType:"",text:""};
    const status=Number(r.status||0);
    const finalUrl=clean(r.url)||clean(url);
    let contentType="";
    try { contentType=clean(r.headers&&r.headers.get?r.headers.get("content-type"):""); } catch(_) {}
    if(!r.ok) return {ok:false,status,error:`HTTP ${status||"ERR"}`,url:finalUrl,contentType,text:""};
    let text="";
    if(/mpegurl|m3u8|text\//i.test(contentType) || /\.m3u8(?:$|[?#])/i.test(finalUrl)) {
      try { text=await r.text(); } catch(e) { return {ok:false,status,error:`body ${clean(e&&e.message?e.message:e)||"read failed"}`,url:finalUrl,contentType,text:""}; }
      if(text.length>160000) text=text.slice(0,160000);
    }
    return {ok:true,status,error:"",url:finalUrl,contentType,text};
  } catch(e) {
    if(timer) clearTimeout(timer);
    const msg=clean(e&&e.message?e.message:e)||"transport error";
    return {ok:false,status:0,error:/abort/i.test(msg)?"timeout":msg,url:clean(url),contentType:"",text:""};
  }
}
...
function normalizeDecryptedUrl(text,depth){
  ...
  if(/%[0-9a-f]{2}/i.test(raw)) { try { const decoded=decodeURIComponent(raw); if(decoded!==raw){ const nested=normalizeDecryptedUrl(decoded,level+1); if(nested) return nested; } } catch(_) {} }
  if(looksBase64(raw)) { try { const decoded=utf8Decode(base64ToBytes(raw)); if(decoded && decoded!==raw){ const nested=normalizeDecryptedUrl(decoded,level+1); if(nested) return nested; } } catch(_) {} }
  const quoted=unescaped.match(/https?:\/\/[^\s"'\\]+/i); if(quoted) return clean(quoted[0]);
  const protoRelative=unescaped.match(/\/\/[A-Za-z0-9._-]+[^\s"'\\]*/); return protoRelative ? `https:${clean(protoRelative[0])}` : "";
}
function previewPlaintext(text){ const raw=clean(text).replace(/[\x00-\x1f\x7f]/g,"?").replace(/\s+/g," "); return short(raw,72); }
...
async function decryptVidrock(blob){
  try {
    ...
    if(typeof Uint8Array==="undefined") return {ok:false,error:"Uint8Array unavailable"};
    ...
    return {ok:true,url,shape,preview:previewPlaintext(text)};
  } catch(e) { return {ok:false,error:clean(e&&e.message?e.message:e)||"decrypt failed"}; }
}

function resolveUrl(base, relative){ try{return new URL(clean(relative),clean(base)).toString();}catch(_){return clean(relative);} }
function parseMasterPlaylist(text, baseUrl){
  const lines=String(text||"").split(/\r?\n/); const variants=[];
  for(let i=0;i<lines.length;i++){
    const line=clean(lines[i]); if(!/^#EXT-X-STREAM-INF:/i.test(line)) continue;
    const attrs=line.slice(line.indexOf(":")+1); let uri="";
    for(let j=i+1;j<lines.length;j++){ const next=clean(lines[j]); if(!next)continue; if(next[0]==="#")continue; uri=next; break; }
    if(!uri) continue;
    const res=attrs.match(/RESOLUTION=\d+x(\d+)/i); const bw=attrs.match(/(?:AVERAGE-)?BANDWIDTH=(\d+)/i);
    variants.push({url:resolveUrl(baseUrl,uri),height:res?Number(res[1]):inferHeight(uri),bandwidth:bw?Number(bw[1]):0});
  }
  variants.sort((a,b)=>(b.height-a.height)||(b.bandwidth-a.bandwidth)); return variants;
}
...
async function vidrockProbe(tmdbId,type,s,e){
  ...
  if(!r.ok)return {ok:false,error:r.error||`HTTP ${r.status||"ERR"}`,entries:[],streams:[],errors:[]};
...
async function resolveVidrockTransport(stream){
  const first=await fetchTransport(stream.url,vidrockPlaybackHeaders());
  if(!first.ok) return {...stream,playable:false,transport:`${first.error}${first.status?` (${first.status})`:""}`,http:first.status||0,finalUrl:first.url||stream.url};
  const body=clean(first.text); const ct=clean(first.contentType).toLowerCase();
  if(/text\/html|application\/xhtml/i.test(ct) || /^<!doctype html|^<html/i.test(body)) return {...stream,playable:false,transport:"HTML RESPONSE",http:first.status,finalUrl:first.url||stream.url};
  if(/^#EXTM3U/i.test(body)) {
    if(/#EXT-X-STREAM-INF:/i.test(body)) {
      const variants=parseMasterPlaylist(body,first.url||stream.url); if(!variants.length) return {...stream,playable:false,transport:"HLS MASTER variants=0",http:first.status,finalUrl:first.url||stream.url};
      const best=variants[0];
      const second=await fetchTransport(best.url,vidrockPlaybackHeaders());
      if(!second.ok) return {...stream,playable:false,transport:`HLS MASTER variants=${variants.length} • variant=${second.error}`,http:second.status||first.status,finalUrl:best.url};
      const secondBody=clean(second.text); if(secondBody && !/^#EXTM3U/i.test(secondBody)) return {...stream,playable:false,transport:`HLS MASTER variants=${variants.length} • variant not HLS`,http:second.status,finalUrl:second.url||best.url};
      return {...stream,playable:true,url:second.url||best.url,height:best.height||inferHeight(best.url),transport:`HLS MASTER variants=${variants.length}`,http:first.status,finalUrl:second.url||best.url};
    }
    return {...stream,playable:true,url:first.url||stream.url,height:inferHeight(first.url||stream.url),transport:"HLS MEDIA",http:first.status,finalUrl:first.url||stream.url};
  }
  if(/^video\//i.test(ct) || /\.mp4(?:$|[?#])/i.test(first.url||stream.url)) return {...stream,playable:true,url:first.url||stream.url,height:inferHeight(first.url||stream.url),transport:`DIRECT ${ct||"video"}`,http:first.status,finalUrl:first.url||stream.url};
  return {...stream,playable:false,transport:`UNKNOWN ${ct||"content"}`,http:first.status,finalUrl:first.url||stream.url};
}

function streamRow(tmdbId,type,s,e,server,url,language,headers,height){ const h=Number(height||inferHeight(url)||0); return {name:`${PROVIDER_NAME} • ${tier(h)} • ${server}`,title:`TMDB ${tmdbId}${type==="tv"?` • S${s}E${e}`:" • Movie"}`,url,quality:h?`${h}p`:"Auto",language:language||"Auto",headers:headers||{"User-Agent":USER_AGENT},provider:PROVIDER_NAME,type:/\.mp4(?:$|[?#])/i.test(url)?"mp4":"m3u8",subtitles:[]}; }
...
async function getStreams(inputId,mediaType="tv",season=1,episode=1){
  ...
  for(const p of vidzee){ if(!p.ok){failures.push(`${p.server}=${p.error}`);continue;} vidzeeOK++; if(!seen.has(p.url)){seen.add(p.url);rows.unshift(streamRow(tmdbId,type,s,e,`VidZee ${p.server}`,p.url,p.language,{"User-Agent":USER_AGENT,"Referer":`${VIDZEE_ORIGIN}/`,"Origin":VIDZEE_ORIGIN},0));} }
  ...
  if(!vidrock.ok) rows.push(diag("VIDROCK FAIL",vidrock.error));
    ...
    rows.push(diag("VIDROCK API",`HTTP 200 • entries=${vidrock.entries.length} • decrypted=${vidrock.streams.length}${vidrock.errors.length?` • ${vidrock.errors.slice(0,3).join(" | ")}`:""}`));
    const resolved=await Promise.all(vidrock.streams.slice(0,5).map(resolveVidrockTransport));
    for(const p of resolved){
      rows.push(diag("VIDROCK STREAM",`${p.server} • HTTP ${p.http||0} • ${p.transport} • host=${hostOf(p.finalUrl||p.url)}`));
      if(!p.playable || seen.has(p.url)) continue;
      seen.add(p.url); rows.unshift(streamRow(tmdbId,type,s,e,`Vidrock ${p.server}`,p.url,p.language,vidrockPlaybackHeaders(),p.height));
    }
  ...
  if(!seen.size) rows.push(diag("NO STREAM",`Catalog matched, but VidZee/Vidrock returned no validated playable source`));
  return rows;
}
