...
const PROVIDER_NAME = "Re:ANIME Test Direct";
...
async function resolveDirectDownload(server) {
  ...
  console.log(`[Re:ANIME Test Direct] ${audio.tag} ${displayServerName(server, "Direct")} direct=${fileId && token ? "yes" : "no"} resolution=${resolution}`);

  if (!fileId || !token) return null;
  const serverName = displayServerName(server, "Direct");

  return {
    name: `${PROVIDER_NAME} • ${serverName} • ${resolution} • ${audio.language} [${audio.tag}] • MKV DIRECT TEST`,
...
async function getStreams(inputId, mediaType = "tv", season = 1, episode = 1) {
  try {
    ...
    const settled = await Promise.all(servers.map(server => resolveDirectDownload(server).catch(() => null)));
    ...
    console.log(`[Re:ANIME Test Direct] ${error && error.message ? error.message : error}`);
    return [];
  }
}
