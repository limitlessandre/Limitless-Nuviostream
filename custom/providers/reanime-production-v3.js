...
// Keeps the validated provider untouched and only normalizes user-facing quality labels.
...
function normalizeRow(row) {
  ...
  if (height) {
    rest = rest.replace(new RegExp(`\\b${height}p\\b`, "i"), "");
  }
  rest = rest
  ...
  return {
    ...row,
    name: `${PROVIDER_NAME} • ${label}${rest ? ` • ${rest}` : ""}`
  };
}
