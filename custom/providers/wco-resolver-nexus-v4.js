...
// Nexus-only generic WCO resolver validation layer.
// Builds on the validated v2 resolver without touching production WCO.
// v4 makes the Sailor Moon fixes deterministic: short TMDB season codes are
// combined with the show title, direct title slugs are safe search candidates,
// exact title matches outrank franchise extensions, and unrelated sidebar links
// are rejected before episode matching.
...
function patchV2Source(source) {
  ...
  // Patch the v2 season-title template itself. This happens before v2 evaluates,
  // so one-letter TMDB season codes such as R/S cannot become standalone searches.
  out = out.replace(
    'if(ns&&nh&&(ns.includes(nh)||nh.includes(ns)))out.push({title:season,kind:"season",sourceSeason});',
    'if(ns&&nh&&ns.length>=3&&(ns.includes(nh)||nh.includes(ns)))out.push({title:season,kind:"season",sourceSeason});'
  );
  out = out.split('out.push({title:season,kind:"season",sourceSeason});')
    .join('if(normalize(season).length>=3)out.push({title:season,kind:"season",sourceSeason});');

  // Add stable runtime patches to v2's patchResolver. These operate on the v1
  // resolver source after it is fetched, avoiding another nested wrapper layer.
  const head = '  if (!out) return "";';
  if (!out.includes(head)) return "";
  const runtime = [
    head,
    '  out = out.replace(\'    if (type === "movie" || Number(info.id) !== 2328) return [];\', \'    if (type === "movie") return [];\');',
    '  out = out.replace(\'      const score=scoreTitle(item.title,title);\', \'      let score=scoreTitle(item.title,title);\\n      const __q=normalize(title),__i=normalize(item.title);\\n      if(__q&&__i===__q)score=Math.max(score,140);\\n      else if(__q&&__i.startsWith(__q+" "))score=Math.max(0,score-30);\');',
    '  out = out.replace(\'  return all.sort((a,b)=>b.score-a.score).slice(0,8);\', \'  const __slug=String(title||"").toLowerCase().replace(/&amp;|&/g," and ").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");\\n  if(__slug){for(const __origin of ORIGINS){const __href=__origin+"/anime/"+__slug+"/?season=all";if(!all.some(x=>x.href===__href))all.push({href:__href,title,score:130,direct:true});}}\\n  return all.sort((a,b)=>b.score-a.score).slice(0,8);\');',
    '  const __episodeLinkMarker = \'    const href=absolute(m[1],pageUrl);\\n    if(!href||!text||/\\/anime\\//i.test(href))continue;\';',
    '  const __episodeLinkGuard = \'    const href=absolute(m[1],pageUrl);\\n    if(!href||!text||/\\/anime\\//i.test(href))continue;\\n    const __seriesSlug=(String(pageUrl||"").match(/\\/anime\\/([^/?#]+)/i)||[])[1]||"";\\n    if(__seriesSlug){const __stop={watch:1,online:1,anime:1,cartoon:1,english:1,dub:1,sub:1};const __tokens=normalize(__seriesSlug).split(" ").filter(w=>w.length>2&&!__stop[w]);const __hrefWords=normalize(href).split(" ");const __hits=__tokens.filter(w=>__hrefWords.includes(w)).length;const __need=Math.max(1,Math.ceil(__tokens.length*0.6));if(__tokens.length&&__hits<__need)continue;}\';',
    '  if(out.includes(__episodeLinkMarker))out=out.split(__episodeLinkMarker).join(__episodeLinkGuard);'
  ].join("\n");
  out = out.replace(head, runtime);
