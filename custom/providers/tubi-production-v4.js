...
// Tubi Nexus presentation wrapper using NAMING_STANDARDS.md.
// Tubi extraction, anonymous auth, anime matching, diagnostics, and Power Rangers
// mapping remain in tubi-production-v3. This file only normalizes stream labels.
const PROVIDER_NAME = "Tubi";
const BASE_URL = "https://raw.githubusercontent.com/limitlessandre/Limitless-Nuviostream/refs/heads/Limitless-nexus/custom/providers/tubi-production-v3.js";
let cached = null;
...
function mirrorLabel(row) {
  const m = String(row && row.name || "").match(/\bmirror\s*(\d+)\b/i);
  return m ? `Mirror ${m[1]}` : "";
}
