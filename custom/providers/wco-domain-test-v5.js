...
async function patchedModule() {
  ...
  const replacement = [
    ...
    '            }',
    '            return{streams:[],stage:"PREMIUM EMBED",detail:variant+": HTTP "+(eRes.status||200)+" final="+originOf(eUrl)+" inner="+(eInner?originOf(eInner):"none")+" markers="+eHints};',
    '          }',
    '          const resolved=await __pdResolvePremiumPage(pUrl,premium.text,variant,ep.text||info.title,info);'
  ].join("\n");
...
async function getStreams(inputId, mediaType, season, episode) {
  ...
  if (!mod) return [{
    name: PROVIDER_NAME + " • DIAG MODULE • premium embed diagnostic wrapper failed to load",
...
function onSettings() {
  return [
    ...
    { type: "info", label: "Premium testing now follows the authenticated free.wcopremium.tv page into embed.wcopremium.tv and reports that inner player's structure. No authentication or entitlement checks are bypassed." },
